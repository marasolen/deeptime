...
const goToEvent = (groupIndex, eventIndex) => {
    console.log(groupIndex, eventIndex)
    backGroupAmount = currentGroupIndex - groupIndex;
    backEventAmount = data[groupIndex].events.length - eventIndex;

    const event = data[groupIndex].events[eventIndex];
    timeline.updateData(event, null, animationDuration);
    tieredTimeline.setBoldedEvent(groupIndex, eventIndex, false);
