...
let buttonUnclickTimer; 
const autoClickButton = (buttonId) => {
    clearTimeout(buttonUnclickTimer)
    const button = document.getElementById(buttonId)
    button.click();
    button.classList.toggle("clicked", true)

    buttonUnclickTimer = setTimeout(() => button.classList.toggle("clicked", false), 60);
}
...
window.addEventListener('load', async () => {
    ...
    document.getElementById("tap-handler").addEventListener("click", () => {
        autoClickButton("next-button");
    });

    document.getElementById("wait-tap-handler").addEventListener("click", () => {
        autoClickButton("wait-button");
    });

    document.getElementById("pause-tap-handler").addEventListener("click", () => {
        autoClickButton("pause-button");
    });
